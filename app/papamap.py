...
def get_nearest_timezome():
    ...
    df["timezone"] = df["timezone"].apply(lambda x: x.replace("_", " "))\
        .apply(lambda x: f"{x.split('/')[-1]} ({x.split('/')[0]})" if len(x.split("/")) > 1 else x)
...
def create_map(meridian):
    ...
    if places.shape[0] > 0:
        places["label"] = places.apply(lambda x:
                                       f"<span style='font-family: Arial; line-height: 1.5rem;'>"
